...
До прикладу 7 тест впав через банальну очепятку
...
> _Вписати тут..._
"list використовую локально щоб одразу в терміналі бачити де падаемо"
"html потрібен коли треба детально переглянути умову на якій впали"
"junit генерує XML-файл для аналізу СІ системами, загрузками у Jenkins, GitHub Actions"

**Аналіз впалого тесту:**
7 тест 
- **Що** впало: перевірка умови пошуку
- **Як** впало: не знайшло елементу
- **Чому** (root cause): через некоректну назву(сплутали порядок літер)

Header 3 (logo)	Regex /greencity-logo/i не збігається з реальним alt "Image green city logo"
Header 7 (login)	замість link зробили img
Header 10 (search)	Input пошуку прихований за замовчуванням, toBeVisible() падає на невидимому елементі
Main 1 (heading)	Текст заголовку відрізняється: тест шукає "grow habits", реальний — "cultivate useful habits"
Main 4 (newsletter)	fill('test123@example.com') але toHaveValue('test@example.com') — різні значення
Footer 1 (nav links)	Текст лінків не збігається точно з масивом у тесті
Footer 2 (social)	Опечатка: 'leenkedin' замість 'linkedin'
---

## Завдання 2 — Перевірити свій аналіз через AI-агента *(опційно)* - перевірив)
...
> _Вставити текст або посилання на скриншот..._ у директорії резалтів 2 скрін

**Порівняння з власним аналізом (3–5 речень):**
> _Вписати тут..._
половину я точно знайшов сам і доволі легко, деякі падіння навпаки були складні і чесно користувався допомогою клода, він у мене налаштований як педагог з самого початку. тож не вирішуе а лише направляю в сторону правельної відповіді.
**Оцінка дотримання обмежень:**

> _Вписати тут..._
я ШІ налаштував трохи раніше власним промтом і дотримуєтся інструкції чудово
---
...
тест 3
було (/greencity-logo/i) стало (/green city logo/i)
тест 7
було шукали по link а треба по img
тест 10 - найважчий виявився
для того шоб побачити елемент спершу траба на нього клік
await page.getByAltText('Internal search button').click();
тест 1
тут просто був не корректний текст пошуку. знайшли корректний - page.getByRole('heading', { name: 'A new way to cultivate useful' }. згодом виправив на         page.getByRole('heading', { name: /A new way to cultivate useful/i }),
тест 4 
вводили і шукали різні значення
await emailInput.fill('test123@example.com');
      await expect(emailInput).toHaveValue('test123@example.com');
тест 1 футер
- змінив пошук самого футеру
- через те що в деяких назвах є пробіл то поставив регекс
- додав 6 елемент
тест 2 футер
- leenkedin > linkedin
...
> _Вписати тут..._
не всі, але більшість відразу так
---
